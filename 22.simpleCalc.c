...
int main(void){
	...
	while(1){
		printf("수식을 입력하세요 : ");
		scanf("%d %c %d", &x, &o, &y); // scanf() : 해당 함수를 사용할 때 변수 이름 앞에 '&'를 붙여야 합니다 / 형식 지정자 %c : 엔터(enter)도 하나의 공백 문자로서 입력 받을 수 있습니다. 
		if(o == '+'){
			printf("%d %c %d = %d\n", x, o, y, x + y);
		}else if(o == '-'){
			printf("%d %c %d = %d\n", x, o, y, x - y);
...
/*
	...
	!주의!
	 가끔 버퍼가 프로그램 동작을 방해합니다. C언어에서는 간단하게 getchar()함수로 버퍼의 공백 문자를 받아낼 수 있습니다.
*/
